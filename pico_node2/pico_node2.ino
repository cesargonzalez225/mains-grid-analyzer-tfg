...
void sendWarning(uint8_t code, const String &text) {
  ...
  String packet = "W|" + String(NODE_ID) + "|" + String(radioCounter) + "|" + String(code) + "|" + text;
...
void relayMessage(int32_t originNodeId, uint32_t counter, uint8_t code, const String &text) {
  if (!radioOk) return;
  ...
  String packet = "W|" + String(originNodeId) + "|" + String(counter) + "|" + String(code) + "|" + text;
...
void handleReceivedPacket() {
  ...
  String text = packet.substring(p4 + 1);
  ...
  if (!HAS_SENSOR && originNodeId != NODE_ID) {
    relayMessage(originNodeId, counter, code, text);
  }
